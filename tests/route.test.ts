...
describe('Test Path Builder', () => {
    ...
    test('Build path with leading slash', () => {
        const result = buildPath('/contacts/:id/name/:name', { id: '3', name: 'test' });
        expect(result).toBe('/contacts/3/name/test');
    });

    test('Build path without leading slash', () => {
        const result = buildPath('contacts/:id/name/:name', { id: '53', name: 'name' });
        expect(result).toBe('contacts/53/name/name');
    });

    test('Build path with leading param', () => {
        const result = buildPath(':id/contacts/name/:name', { id: '26', name: 'name' });
    ...
    test('Build path with one query', () => {
        const result = buildPath('/contacts/:id/name/:name', { id: '2', name: 'test', query1: 'with<and&symbols' });
